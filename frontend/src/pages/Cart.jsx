      ...
      for(const x in cartItems){
        for(const y in cartItems[x])
        {
          if(cartItems[x][y]>0)
          {
            tempData.push({
              _id:x,// id of Product
              size:y,//size of product 
              quantity:cartItems[x][y] // quantity of product 
            })
          }
        }
      }
                ...
                <div className=" flex items-start gap-6">
                  ...
                  <div>
                    ...
                    <div className=" flex items-center gap-5 mt-2">
                      ...
                      <p className=' px-2 sm:px-3 sm:py-1 border text-white bg-gray-950'>{item.size}</p>
                    </div>
                  </div>
                </div>
      ...
      <div className=" flex justify-end my-20">
        <div className=" w-full sm:w-[450px] ">
          ...
          <div className=" w-full text-end">
            <button onClick={()=>navigate('/placeorder')} className=" bg-black text-white text-sm my-8 px-8 py-3 active:bg-gray-700 ">PROCCED TO CHECKOUT</button>
          </div>
        </div>
      </div>
